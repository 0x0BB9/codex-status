...
pub mod private {
  ...
  pub trait IntoInvokeError {
    fn into_invoke_error(self) -> InvokeError;
  }

  impl<T: Serialize> IntoInvokeError for T {
    #[inline]
    fn into_invoke_error(self) -> InvokeError {
  ...
  impl IntoInvokeError for InvokeError {
    #[inline]
    fn into_invoke_error(self) -> InvokeError {
      self
    }
  ...
  impl IntoInvokeError for crate::Error {
    #[inline]
    fn into_invoke_error(self) -> InvokeError {
      self.into()
    }
  ...
  impl<T: IpcResponse, E: IntoInvokeError> ResultKind for Result<T, E> {}
  ...
  impl ResultTag {
    ...
    where
      ...
      E: IntoInvokeError,
    {
      resolver.respond(value.map_err(IntoInvokeError::into_invoke_error))
    }
    ...
    where
      ...
      E: IntoInvokeError,
    {
      Ok(value.map_err(IntoInvokeError::into_invoke_error)?.body()?)
    }
  }
  ...
  impl<T: IpcResponse, E: IntoInvokeError, F: Future<Output = Result<T, E>>> ResultFutureKind
    for F
  {
  }
  ...
  impl ResultFutureTag {
    ...
    {
      let response = value.await.map_err(IntoInvokeError::into_invoke_error)?;
      Ok(response.body()?)
    }
  }
}
