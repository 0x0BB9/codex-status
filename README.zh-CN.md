...
- 导入或手动添加 OpenAI Responses 兼容 API 连接，并在 Plus 与 API 连接之间切换
- 标记失效账号，并支持重新登录或安全删除非当前账号
- 展示五小时和一周额度的使用比例、重置时间，以及可用重置次数和最近到期时间
- 根据最近活动时间排列任务
...
- 提供多套浅色主题，并记住用户选择
- Windows 与 macOS 均支持应用内检查、下载、安装新版并自动重启
- 更新重启后显示一次升级结果和本次更新内容
...
- 点击 Plus 账号的“切换”后，应用会先保存当前账号的最新 token，再替换 `auth.json`，然后刷新 Codex 并验证目标账号。
- 如果验证失败，应用会自动恢复原账号，避免 Codex 停留在退出登录状态。
- 验证失败的目标账号会标记为“需要重新登录”；重新登录成功后自动清除标记。
- 非当前账号可以二次确认删除；当前正在使用的账号不会提供删除入口。
- API Key 长期保存在 macOS 钥匙串或 Windows 凭据管理器；账号列表只记录末四位提示，不保存明文 Key。
- 激活 API 连接时，应用会保留 Plus 登录，切换模型 provider 和本地模型目录，并自动打开 provider 正确的独立新任务；切回 Plus 时恢复原有 Codex 配置。
- API Key 激活期间只会临时写入权限为当前用户可读的 provider 配置，连接备份会剔除该 bearer token。
- 切换前后会读取全部 provider 的未归档线程数量；如果线程数量减少，应用会停止切换并自动恢复原连接。
...
- Windows 与 macOS 检查到新版后，均可在应用内下载安装，安装完成后自动重启。
- 更新重启后会显示一次来源版本、目标版本和 Release Notes，并提供完整日志入口。
- macOS 使用应用内更新前，请先把 App 复制到 `/Applications`，不要直接从 DMG 中运行。
- 更新包使用独立的 Tauri 签名进行完整性验证；该签名与 Windows 发布者证书、macOS Developer ID 是不同的安全机制。
- macOS 首次升级到支持自动安装的版本时仍需手动替换一次；之后才能使用应用内更新。
...
五小时和一周额度、账号及任务数据仍来自官方 `codex app-server`。由于当前 app-server 协议不提供可用重置次数，应用仅为这一项读取当前 `auth.json` 的登录态，并只读请求 ChatGPT 的额度重置端点。Token、账号 ID 和原始响应不会写入应用状态文件或日志；接口不可用或返回结构变化时，只隐藏重置次数，不影响其他看板功能。
